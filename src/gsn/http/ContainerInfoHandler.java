...
logger.error("in CIH");
  ...
  logger.error( "The handle was called" );
 ...
 // if (user != null) System.out.println("User " + user.getUserName());
  ...
  public String buildOutput (String reqName, User user) {
   ...
   logger.error( "Inside buildOutput -- name = "+sensorConfig.getName());
      if(Main.getContainerConfig().isAcEnabled())
      {
          if (user != null)
          {
              logger.error( "The user is defined"+sensorConfig.getName());
              if ( (reqName != null && !sensorConfig.getName().equals(reqName) )|| ( user.hasReadAccessRight(sensorConfig.getName())== false && user.isAdmin()==false) ) {//continue;
                                   access = false;
                  logger.error("Source = "+sensorConfig.getName()+" has access = "+ access);
              }
          }
          else {
              logger.error("Datasource - "+DataSource.isVSManaged(sensorConfig.getName()));
      ...
      if (access == true) {
          ...
          location.append("\n");
          logger.error("Loc = "+location.toString() );
  ...
  public static ArrayList<StreamElement> getMostRecentValueFor(String virtual_sensor_name) {
    logger.error("GET NEW FOR = "+virtual_sensor_name);
