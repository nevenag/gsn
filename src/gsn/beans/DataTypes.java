...
public class DataTypes {
	...
	// NEXT FIELD
	public final static String CHAR_PATTERN_STRING = "\\s*char"
			+ REQUIRED_NUMBER_PARAMETER + "\\s*";
	...
	// NEXT FIELD
	public final static String BINARY_PATTERN_STRING = "\\s*(BINARY|BLOB)"
			+ OPTIONAL_NUMBER_PARAMETER + "(\\s*:.*)?";
	...
	public final static Object[] TYPE_SAMPLE_VALUES = { "A chain of chars",
			'c', new Integer(32), new Integer(66000), new Byte((byte) 12),
	...
	public static byte convertTypeNameToGSNTypeID(final String type) {
		...
		throw new GSNRuntimeException(new StringBuilder("The type *")
				.append(type).append("* is not recognized.")
	...
	public static byte SQLTypeToGSNTypeSimplified(int sqlType) {
		if (sqlType == Types.BIGINT || sqlType == Types.SMALLINT
				|| sqlType == Types.DOUBLE || sqlType == Types.INTEGER
				|| sqlType == Types.DECIMAL || sqlType == Types.REAL
			...
			return DataTypes.VARCHAR;
		else if (sqlType == Types.BINARY || sqlType == Types.BLOB
				|| sqlType == Types.VARBINARY)
			return DataTypes.BINARY;
		throw new RuntimeException("Can't convert SQL type id of: " + sqlType
				+ " to GSN type id.");
	}
}
